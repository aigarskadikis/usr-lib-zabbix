...
echo $OIDDESCRIPTION | grep "CRITICAL" && \
echo $OIDNAME && \
echo $OID && \
...
curl --silent --insecure --request POST --header 'Content-Type: application/json-rpc' --data "
{
    ...
    \"params\": [
        {
            \"description\": \"$(echo "$OIDDESCRIPTION" | sed 's|^CRITICAL:\s*||;s|\.*$||')\",
...
curl --silent --insecure --request POST --header 'Content-Type: application/json-rpc' --data "
{
    ...
    \"params\": [
        {
            \"description\": \"$(echo "$OIDDESCRIPTION" | sed 's|^WARNING:\s*||;s|\.*$||')\",
            \"expression\": \"{$HOST_NAME:snmptrap[\\\"\\\\s$OID_ESC\\\\s\\\"].nodata(1d)}=0\",
                        \"priority\":\"3\"
        }
    ],
...
##############################################################
# SNMP traps - trap description contains keyword INFORMATION
# SNMP traps - automaticallt generate trigger if trap description contains keyword 'INFORMATION'
##############################################################
...
OID_ESC=$(echo .$OID | sed 's|\.|\\\\.|g') && \
OIDDESCRIPTION=$(snmptranslate -m ./$MIB .$OID -Td | tr -d '\n' | sed 's|^.*DESCRIPTION\s*||' | sed -e's/  */ /g' | sed 's|^\d034||' | sed 's|\d034||g' | sed 's|::=.*$||') && \
echo $OIDDESCRIPTION | grep "INFORMATION" && \
echo $OIDNAME && \
echo $OID && \
echo \"key_\": \"snmptrap[\\\"\\\\s$OID_ESC\\\\s\\\"]\", && \
curl --silent --insecure --request POST --header 'Content-Type: application/json-rpc' --data "
...
curl --silent --insecure --request POST --header 'Content-Type: application/json-rpc' --data "
{
    ...
    \"params\": [
        {
            \"description\": \"$(echo "$OIDDESCRIPTION" | sed 's|^INFORMATION:\s*||;s|\.*$||')\",
            \"expression\": \"{$HOST_NAME:snmptrap[\\\"\\\\s$OID_ESC\\\\s\\\"].nodata(1d)}=0\",
                        \"priority\":\"1\"
        }
    ],
